...
ml.create <- plot(ml.create.pred) +
  theme(text = element_text(size = 20)) +
  ggtitle(sprintf("showing when a function is fast and \nwhen a function is Slow", n.folds)) +
  scale_x_log10("N") +
  scale_y_log10("Computation time (seconds)") +
  facet_null() +
  scale_fill_manual(values = ml.colors) +
...
```{r}
print(ml.create)
ggsave("ml.create.png", ml.create, width = 6, height = 3, units = "in", dpi = 100)
...
ml.exp.gg <- plot(ml.exp.pred)+
  theme(text=element_text(size=20))+
  ggtitle(sprintf("Test Example to show Slow and Fast for Comparative Benchmarking", n.folds))+
  scale_x_log10("N")+
  scale_y_log10("Computation time (seconds)
median line, min/max band
over 10 timings")+
