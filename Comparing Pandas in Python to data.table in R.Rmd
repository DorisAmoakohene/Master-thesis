...
```{r}
png("atime_overhead.png", res = 600, width = 5, height = 3, unit = "in") 
plot(atime.overhead)  
dev.off()
```
...
```{r}
# Compute overhead in seconds
(overhead.seconds <- median(atime.overhead$measurements$median, na.rm = TRUE))
...
```{r}
csv_write_methods_pandas_data.table <- plot(pred.write.vary.cols) +
...
print(csv_write_methods_pandas_data.table)
```

```{r}
png()
print(csv_write_methods_pandas_data.table)
ggsave("csv_write_methods_pandas_data.table.png", csv_write_methods_pandas_data.table, width = 10, height = 6, units = "in", dpi = 300)
dev.off()
```
...
```{r,warning=FALSE,message=FALSE}
csv_read_methods_pandas_data.table <- plot(pred.read.vary.cols)+
  theme(text=element_text(size=15))+
...
```{r,warning=FALSE,message=FALSE}
plot(csv_read_methods_pandas_data.table)

png()
print(csv_read_methods_pandas_data.table)
ggsave("csv_read_methods_pandas_data.table.png", csv_read_methods_pandas_data.table, width = 10, height = 6, units = "in", dpi = 300)
dev.off()

csv_read_methods_pandas_data.table
```
...
```{r,warning=FALSE,message=FALSE}
ml_reshape_wide_to_long_pandas_data.table <- plot(ml.reshape.pred)+
  theme(text=element_text(size=15))+
...
```{r, warning=FALSE, message=FALSE}
plot(ml_reshape_wide_to_long_pandas_data.table)

png()
print(ml_reshape_wide_to_long_pandas_data.table)
ggsave("ml_reshape_wide_to_long_pandas_data.table.png", ml_reshape_wide_to_long_pandas_data.table, width = 10, height = 6, units = "in", dpi = 300)
dev.off()

ml_reshape_wide_to_long_pandas_data.table
```
...
```{r,warning=FALSE,message=FALSE}
ml_reshape_long_to_wide_pandas_data.table <- plot(ml.long2wide.pred)+
  theme(text=element_text(size=15))+
  ggtitle(sprintf("Reshaping from long to wide, comparing pandas & data.table \nover real numbers, N times", n.folds))+
  scale_x_log10("N")+
  scale_y_log10("Computation time (seconds)
median line, min/max band
over 10 timings")+
...
```{r,warning=FALSE,message=FALSE}
plot(ml_reshape_long_to_wide_pandas_data.table)

ggsave("ml_reshape_long_to_wide_pandas_data.table.png", ml_reshape_long_to_wide_pandas_data.table, width = 10, height = 6, units = "in", dpi = 300)

png()
print(ml_reshape_long_to_wide_pandas_data.table)
ggsave("ml_reshape_long_to_wide_pandas_data.table.png",ml_reshape_long_to_wide_pandas_data.table, width = 10, height = 6, units = "in", dpi = 300)
dev.off()

ml_reshape_long_to_wide_pandas_data.table
```
